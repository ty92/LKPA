...
&emsp;&emsp;Linux是为多用户多任务设计的操作系统, 所以存储资源要被多个进程有效共享；且由于程序规模的不断膨胀，要求的内存空间比从前大得多。Linux内存管理的设计充分利用了计算机系统所提供的虚拟存储技术，真正实现了虚拟存储器管理。为每个进程划分更大的虚拟地址空间，并且将每个进程地址空间又划分为完全独立的用户地址空间和共享的内核虚拟地址空间，最后，通过CPU将进程操作的虚拟地址送至MMU（2.1.3节介绍）进行地址转换，为进程映射真正的物理内存，整个操作系统内存管理结构如图4.2所示。
