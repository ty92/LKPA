...
换出页面时一个非常重要的数据结构就是LRU链表，Linux系统中定义了4个不同类型的LRU链表：活跃/非活跃匿名页面，活跃/非活跃文件映射页面和不可回收页面，每次在需要回收时，可以根据页面的属性
```c
enum lru_list {
...
-   分配页面。调用__get_free_pages()从某个空闲队列分配内存页面，并将其页面的使用计数count置为1。
